# This module defines the API endpoints for managing properties in the real estate platform.
# It includes routes for listing properties with filtering and pagination, searching properties by location,
# fetching featured properties, getting property categories, retrieving a single property by ID,
# creating new properties with image uploads, updating existing properties, and deleting properties.
# The endpoints include validation for input data and handle permissions based on user roles (sellers and admins).
# The module also interacts with the database using SQLAlchemy and handles potential errors gracefully,
# providing clear error messages in the API responses.
...
from app.schemas.property_schema import PropertyResponse, PropertyUpdate, PropertyListResponse
...
def list_properties(
    ...
    try:
        ...
        # Convert SQLAlchemy models to Pydantic and then to dict for JSON response
        properties_data = [PropertyResponse.model_validate(p).model_dump() for p in properties]
...
@router.get(
    ...
    description="Returns all properties owned by the authenticated seller (including pending, approved, rejected). Supports pagination and optional status filter."
)
    ...
    try:
        if current_user.role not in ["seller", "admin"]:
            raise HTTPException(
                ...
                detail="Only sellers or admins can view their own properties."
            )

        query = db.query(Property).filter(Property.seller_id == current_user.id)
        ...
        properties_response = [PropertyResponse.model_validate(p) for p in properties]
        ...
        logger.error(f"Database error in get_my_properties: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to fetch your properties."
        )
    except Exception as e:
        logger.error(f"Unexpected error in get_my_properties: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def search_by_location(
    ...
    try:
        ...
        properties = query.offset(skip).limit(limit).all()
        properties_data = [PropertyResponse.model_validate(p).model_dump() for p in properties]
        return {"total": total, "properties": properties_data}
...
def get_featured(db: Session = Depends(get_db)):
    try:
        ...
        return PropertyResponse.model_validate(featured)
...
def create_property(
    ...
    try:
        if current_user.role not in ["seller", "admin"]:
            raise HTTPException(status_code=403, detail="Only sellers or admins can list properties.")

        existing = db.query(Property).filter(Property.title == title).first()
        if existing:
            raise HTTPException(status_code=400, detail={"field": "title", "error": "Title already exists."})

        validate_images(images)

        # Handle missing location data via geocoding
        if location_lat is None or location_lng is None or not city or not state or not zip_code:
            geo = geocode_address(address)
            if geo:
                location_lat = location_lat or geo["lat"]
                location_lng = location_lng or geo["lng"]
                city = city or geo["city"]
                state = state or geo["state"]
                zip_code = zip_code or geo["zip_code"]
                logger.info(f"Geocoded address '{address}' -> lat={location_lat}, lng={location_lng}")
            else:
                ...
                logger.warning(f"Geocoding failed, using fallback values for property '{title}'")
        ...
        for idx, img in enumerate(images):
            try:
                ...
                db.rollback()
                raise HTTPException(status_code=500, detail={"field": "images", "error": str(e)})
        db.commit()
        db.refresh(new_property)
