...
def create_alert(
    ...
    try:
        ...
        # Safely serialize filters to JSON string
        try:
...
@router.get("/", response_model=list[EmailAlertResponse])
def get_my_alerts(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Get all active alerts for the current user safely."""
    try:
...
def toggle_alert(
    ...
    """Activate or deactivate an existing alert with safe commit transaction."""
    try:
...
def delete_alert(
    ...
    """Delete an email alert safely with strict block rollback protection."""
    try:
        alert = db.query(EmailAlert).filter(
            EmailAlert.id == alert_id,
            EmailAlert.user_id == current_user.id
